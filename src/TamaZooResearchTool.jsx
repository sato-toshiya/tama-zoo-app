          ...
          <div className="grid grid-cols-6 gap-3">
            {[1, 2, 3, 4, 5, 6].map(grade => (
              <button
                ...
                className={`p-3 rounded-lg border-2 text-lg font-bold transition-colors ${formData.grade === grade.toString()
            ...
            {categories.map(category => (
              <div key={category.id}>
                <button
                  ...
                  className={`w-full p-3 rounded-lg border-2 text-left transition-colors ${formData.category === category.id
                      ? 'border-green-500 bg-green-100 text-green-800'
                      : 'border-gray-300 hover:border-green-300'
                    }`}
                >
          ...
          <div className="grid grid-cols-3 gap-4">
            {[
              ...
              <button
                ...
                className={`p-4 rounded-lg border-2 text-center transition-colors ${formData.difficulty === item.level
                    ? 'border-green-500 bg-green-100 text-green-800'
                    : 'border-gray-300 hover:border-green-300'
                  }`}
              >
          ...
          {proposedThemes.map((theme, index) => {
            ...
            if (typeof theme === 'string') {
              return (
                <div
                  key={index}
                  className={`bg-white p-6 rounded-lg shadow-md border-2 cursor-pointer transition-colors ${formData.selectedTheme === theme
            ...
            return (
              <div
                key={theme.id}
                className={`bg-white p-6 rounded-lg shadow-md border-2 cursor-pointer transition-colors ${formData.selectedTheme === theme.title
                    ? 'border-green-500 bg-green-50'
                    : 'border-gray-200 hover:border-green-300'
                  }`}
                onClick={() => selectTheme(theme)}
          ...
          <div className="max-w-4xl mx-auto p-4">
            <div className="flex items-center justify-between">
              {steps.map((step, index) => (
                <div
                  key={step.id}
                  className={`flex items-center gap-2 ${index === currentStep ? 'text-green-600 font-bold' :
                      index < currentStep ? 'text-green-500' : 'text-gray-400'
                    }`}
                >
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${index === currentStep ? 'bg-green-600 text-white' :
